...
function simple {
    ...
    touch payload_simple.json
    # Add scene.txt raw content to JSON.
	cat test03.txt | jq -sR '{scene: .}' > payload_simple.json                                                                          
    # Add texmap.bmp binary to JSON (optional step, required only for some scenes).
	hexdump -ve '1/1 "%u\n"' workload/complex/jpeg_files/leaf.jpg | jq -s --argjson original "$(<payload_complex.json)" '$original * {texmap: .}' > payload_complex.json  
    # Send the request.
	curl -s -X POST http://localhost:8001/raytracer?scols=400\&srows=300\&wcols=400\&wrows=300\&coff=0\&roff=0\&aa=false --data @"./payload_simple.json" > result_simple.txt   
...
function complex {
	...
	hexdump -ve '1/1 "%u\n"' workload/complex/jpeg_files/leaf.jpg | jq -s --argjson original "$(<payload_complex.json)" '$original * {texmap: .}' > payload_complex.json  
    # Send the request.
	curl -s -X POST http://localhost:8001/raytracer?scols=400\&srows=300\&wcols=400\&wrows=300\&coff=0\&roff=0\&aa=false --data @"./payload_complex.json" > result_complex.txt   
    # Remove a formatting string (remove everything before the comma).
